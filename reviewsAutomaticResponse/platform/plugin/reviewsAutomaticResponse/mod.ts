...
export async function reviewAutoRespond(webhook: any) {
    ...
    const reviewRequest = new Request (`https://sbx-api.yextapis.com/v2/accounts/me/reviews/${reviewId}?v=20230401&api_key=${API_KEY}`, {
      method: 'GET',  
      headers: {
    ...
    const request = new Request (`https://sbx-api.yextapis.com/v2/accounts/me/assets?v=20230401&limit=999&api_key=${API_KEY}`, {
      method: 'GET',  
      headers: {
      "content-type": "application/json",
      },
...
export async function respondViaApi(review_id: number, review_response: any) {
  try {
    const requestUri = 'https://sbx-api.yextapis.com/v2/accounts/me/reviews/' + review_id.toString() + '/comments?v=20230401&api_key=' + API_KEY;
